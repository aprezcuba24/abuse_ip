...
class BotManager:
    ...
    def _start_messages(self):
        for filters, callback in self.get_message_config():
            self.dispatcher.add_handler(MessageHandler(filters, self._get_callback(callback)))

    def _init_commands(self):
        filter_group = Filters.chat_type.group | Filters.chat_type.supergroup
        self._retister_command_handlers(self.get_group_commands_config(), filters=filter_group)
        self._retister_command_handlers(self.get_private_commands_config(), filters=Filters.chat_type.private)

    def _init_callback_query_handlers(self):
        for key, value in self.get_callback_query_handlers_config().items():
            self.dispatcher.add_handler(CallbackQueryHandler(self._get_callback(value), pattern=key))
    ...
    def _init_conversation(self, config):
        ...
        if "callback_query_handlers" in config["entry_points"]:
            entry_points += [
                ...
                for key, item in config["entry_points"]["callback_query_handlers"].items()
            ]
        if "commands" in config["entry_points"]:
            entry_points += [
                self._create_command_handler(key, value) for key, value in config["entry_points"]["commands"].items()
            ]
        states = {}
        ...
        return ConversationHandler(entry_points=entry_points, states=states, fallbacks=entry_points)
    ...
    def _retister_command_handlers(self, command_config, **kwargs):
        for key, value in command_config.items():
            self.dispatcher.add_handler(self._create_command_handler(key, value, **kwargs))
    ...
    def _get_callback(self, data):
        return getattr(data[0], data[1]) if isinstance(data, tuple) else data
    ...
    def get_command_description(self) -> None:
        ...
        return [(key, value["description"]) for key, value in commands.items() if "description" in value]

    def _filter_my_commands(self, commands):
        return [(key, str(value["description"])) for key, value in commands.items() if "description" in value]
